...
cat <<'EOF' > /tmp/menu_heredoc
    <item label="BASE_APPNAME">
    <action name="Execute">
      <command>BASE_APPNAME</command>
      <startupnotify>
        <enabled>yes</enabled>
      </startupnotify>
    </action>
...
if [ -d "/usr/share/BASE_APPNAME" ]; then
install_paths+=",/usr/share/BASE_APPNAME"
fi

if [ -d "/usr/opt/BASE_APPNAME" ]; then
install_paths+=",/usr/opt/BASE_APPNAME"
fi

if [ -d "/usr/etc/BASE_APPNAME" ]; then
install_paths+=",/usr/etc/BASE_APPNAME"
fi
if [ -d "/usr/lib/BASE_APPNAME" ]; then
install_paths+=",/usr/lib/BASE_APPNAME"
fi
